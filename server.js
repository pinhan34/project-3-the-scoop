...
function updateComment(url,request){
  const commentID = Number(url.split('/').filter(item => item)[1]);
  ...
  if (!commentID || !requestComment) {
    ...
    savedComment.body = requestComment.articleId || savedComment.articleId;
...
function deleteComment(url){
    const commentID = Number(url.split('/').filter(item =>item)[1]);
    const tobeDeletedComment = database.comments[commentID];
    const response = {};
    if(!commentID || tobeDeletedComment){
      response.status = 400;
    }else if(!tobeDeletedComment){
      response.status = 404;
    }
    else{
      database.comments[commentID] = null;
      database.articles.filter(item => item.commentID != commentID);
      database.users.filter(item => item.commentID != commentID);    
      database.articles.foreach(article => {
        article.commentIds.splice(article.commentIds.indexof(commentID),1);
      });

      database.users.foreach(user => {
        user.commentIds.splice(user.commentIds.indexof(commentID),1);
      });
